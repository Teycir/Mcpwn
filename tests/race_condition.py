...
class RaceConditionTest:
    ...
    def _count_successes(self, results):
        """Count results without error or exists indicators"""
        return sum(
            1 for r in results
    ...
    def _execute_race(self, func_with_args, count=10):
        """Execute requests with tight synchronization"""
    ...
    def _test_creation_collision(self, tool):
        """Test if multiple threads can create the SAME resource"""
        findings = []
        tool_name = tool['name'].lower()

        if not any(k in tool_name for k in ['create', 'insert', 'register',
                                             'make', 'add']):
            return findings
        ...
        for arg in schema.get('properties', {}):
            if any(k in arg.lower() for k in ['path', 'name', 'id', 'key',
                                               'email']):
                target_arg = arg
                break
        ...
        success_count = self._count_successes(results)
        ...
        if success_count > 1:
            findings.append({
                ...
                'tool': tool['name'],
                'severity': 'HIGH',
                'note': f'{success_count}/5 threads created resource',
    ...
    def _test_concurrency_stability(self, tool):
        """Test for database locks and internal errors under load"""
        findings = []
        ...
        lock_errors = [
            r for r in results
            if any(x in str(r).lower() for x in ['lock', 'busy'])
        ]
        internal_errors = [
            r for r in results
            if 'internal error' in str(r).lower()
        ]
